...
builder.Services.AddAkka("mpr-akkanet-a", builder =>
{
    builder
        ...
        {
            Roles = ["server"],
            //SeedNodes = ["akka.tcp://mpr-akkanet-a@localhost:4053"]
        })
        .WithAkkaManagement()
        .WithClusterBootstrap(serviceName: "mpr-akkanet-a", requiredContactPoints: 1)
        ...
        .WithShardRegion<ISensorMessage>("sensors", (sys, reg, res) => s => res.Props<SensorActor>(s),
        ...
        .WithShardRegion<ISensorGroupMessage>("sensorgroups", (sys, reg, res) => s => res.Props<SensorGroupActor>(s),
